...
export const getRecipes = async (userId: string, currentPage: number = 1, pageSize = PAGE_SIZE): Promise<any> => {
  try {
    ...
    //amir
    const res = await axios.get(`34.125.222.127:4000/recipe/getRecipes?userId=${ userId }&currentPage=${ currentPage }&pageSize=${ pageSize }`);
    return {
...
export const insertNewRecipesToDB = async (recipe: IRecipe) => {
  if (recipe) {
    try {
      const res = await axios.post(`34.125.222.127:4000/recipe/createNewRecipe`, {...recipe},{
        headers: {
...
export const insertNewImagesToDB = async (image: File) => {
  try {
    ...
    const res = await axios.post(`34.125.222.127:4000/uploadImage`, formData,
      {
        headers: {
...
export const getTotalNumberOfRecipesForPagination = async (userId: string) => {
  if (userId) {
    try {
      const res = await axios.get(`34.125.222.127:4000/recipe/countUserRecipes`, {
        params: {
          userId
        }
      });
...
export const getUserRecipeLinks = async (userId: string, currentPage: number = 1, pageSize: number = 20) => {
  if (userId) {
    try {
      const res = await axios.get(`34.125.222.127:4000/recipe/getUserRecipeLinks?userId=${ userId }&currentPage=${ currentPage }&pageSize=${ pageSize }`,
        {
          headers: {
            'Authorization': `Bearer ${ getJWTFromLocalStorage() }`
          }
...
export const setUserRecipeLink = async (recipeLink: IRecipeLink) => {
  if (recipeLink) {
    try {
      const res = await axios.post(`34.125.222.127:4000/recipe/setUserRecipeLink`, {...recipeLink},
        {
          headers: {
            'Authorization': `Bearer ${ getJWTFromLocalStorage() }`
          }
...
export const fetchRecipesLogIn = async (dispatch: any, currentPage: number = 1) => {
  const res = await axios.get(`34.125.222.127:4000/recipe/getRecipes`,
...
export const getRecipesById = async (recipeId: string): Promise<any> => {
  try {
    const res = await axios.get(`34.125.222.127:4000/recipe/getRecipes?id=${ recipeId }`);
    return res.data;
  } catch (e: any) {
    console.log("error", e)
  }
...
export const incrementInteractionRecipeCount = async (recipeId: string): Promise<any> => {
  try {
    return await axios.post(`34.125.222.127:4000/recipe/incrementInteractionRecipeCount`, {recipeId: recipeId},
      {
        headers: {
          'Authorization': `Bearer ${ getJWTFromLocalStorage() }`
        }
...
export const fetchSearchRequest = async (searchQuery: string, isPrivate: boolean, userProfileId: string, dispatch: any) => {
  const response = await axios.get(`34.125.222.127:4000/recipe/searchRecipe`,
    {
      params: {
